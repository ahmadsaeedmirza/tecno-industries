    ...
    >
      <div className="section-container flex items-center justify-between h-20">
        <Link to="/" className="flex items-center">
          <img src={tecnoLogo} alt="TECNO Instruments" className="h-20 w-auto" />
        </Link>
            ...
            >
              <Link
                ...
                className={`px-3 py-2 text-[15px] font-bold rounded-md transition-all duration-300 inline-flex items-center gap-1 ${
                  location.pathname === l.to || (l.hasDropdown && location.pathname.startsWith("/products"))
                    ? "text-primary"
                ...
                >
                  {categories.map(cat => (
                    ...
                    >
                      ...
                      />
                      <div>
                        <p className="text-xs font-bold text-foreground group-hover:text-primary transition-colors">{cat.name}</p>
                        <p className="text-[10px] text-muted-foreground">{cat.productCount} instruments</p>
                      </div>
                    </Link>
                  ))}
                  <Link
        ...
        <div className="hidden lg:flex items-center gap-2">
          <div className="relative group">
            ...
            />
            <div className="absolute right-2.5 top-1/2 -translate-y-1/2 text-[9px] font-medium text-primary/30 group-focus-within:opacity-0 transition-opacity pointer-events-none">⌘K</div>
          </div>
                ...
                {languages.map(lang => (
                  <button
                    ...
                    className={`w-full text-left px-3 py-1.5 text-xs transition-colors ${
                      currentLang === lang.code ? "text-primary font-bold" : "text-foreground hover:text-primary hover:bg-muted"
                    }`}
                  >
                    {lang.label}
                  </button>
