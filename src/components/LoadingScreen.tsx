...
const criticalImages = [
  () => import("@/assets/hero-slide-1.jpg"),
  () => import("@/assets/hero-slide-2.jpg"),
  () => import("@/assets/hero-slide-3.jpg"),
  () => import("@/assets/hero-slide-4.jpg"),
];
...
const preloadVideo = (src: string): Promise<void> =>
  new Promise((resolve) => {
    ...
    video.oncanplaythrough = () => { clearTimeout(timer); resolve(); };
    video.src = src;
  });
...
const LoadingScreen = ({ onComplete }: { onComplete: () => void }) => {
  ...
  useEffect(() => {
    ...
    const loadAssets = async () => {
      try {
        const imageModules = await Promise.all(criticalImages.map((fn) => fn()));
        const imageSrcs = imageModules.map((m) => m.default);
        const heroVideoModule = await import("@/assets/hero-video.mp4");

        const allTasks = [
          ...imageSrcs.map((src) => preloadImage(src)),
          preloadVideo(heroVideoModule.default),
        ];
        ...
        const tracked = allTasks.map((p) =>
          p.then(() => {
            ...
            setProgress(Math.round((loadedAssets.current / totalAssets.current) * 100));
          })
        );
      ...
      {!exiting && (
        ...
        >
          ...
          <div
            ...
            style={{
              background: "radial-gradient(ellipse at center, hsl(24, 60%, 12%) 0%, transparent 60%)",
            }}
          />
          ...
          <motion.h1
            ...
            style={{
              color: "hsl(24, 85%, 48%)",
              textShadow: "0 0 40px hsl(24, 85%, 48% / 0.3), 0 0 80px hsl(24, 85%, 48% / 0.1)",
            }}
          >
            TECNO
          </motion.h1>
          ...
          >
            <div className="w-40 sm:w-52 md:w-64 h-[3px] rounded-full bg-white/10 overflow-hidden">
              <motion.div
                ...
                style={{
                  background: "linear-gradient(90deg, hsl(24, 85%, 42%), hsl(24, 85%, 55%), hsl(32, 90%, 58%))",
                }}
                initial={{ width: "0%" }}
